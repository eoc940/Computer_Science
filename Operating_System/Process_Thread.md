...
- 유닉스의 예
  ...
  - 여기서 fork와 exec은 시스템 콜이므로 사용자 프로그램이 직접 프로세스를 생성하는 것이 아니라 운영체제가 해주는 것임
  
![image](https://user-images.githubusercontent.com/67304980/140385846-82e75106-e927-46e6-a97e-37be291a29ae.png)

fork()라는 함수가 호출되는 순간 자식 프로세스는 실행되게 된다. fork()를 실행하면 부모 프로세스는 pid로 양수를 받고 자식 프로세스는 0을 받는다. 이렇게 부모 프로세스와 자식 프로세스를 구분할 수 있다. 

exec() 시스템 콜은 완전히 다른 프로세스로 만들어 주는 역할을 한다

![image](https://user-images.githubusercontent.com/67304980/140387151-7d5f6e0c-9cb2-43fc-a535-b4885f60bd55.png)

wait() 시스템 콜에 대해서 설명하자면 프로세스 A가 wait()시스템 콜을 호출하면 커널은 child가 종료될 때까지 프로세스 A를 sleep시킨다(block상태). child process가 종료되면 커널은 프로세스 A를 깨운다(ready 상태)

![image](https://user-images.githubusercontent.com/67304980/140388435-4afadf72-f315-404a-ae5c-8315074d2e03.png)




exit() 시스템 콜에 대해 알아보자. 프로세스의 종료는 자발적 종료와 비자발적 종료로 나뉜다. 자발적 종료는 마지막 statement 수행 후 exit() 시스템 콜을 통해 수행된다. 프로그램에 명시적으로 적어주지 않아도 main 함수가 리턴되는 위치에 컴파일러가 넣어준다. 비자발적 종료는 부모 프로세스가 자식 프로세스를 강제 종료시키는 것이다. 자식 프로세스가 한계치를 넘어서는 자원을 요청하거나 자식에게 할당된 태스크가 더 이상 필요하지 않을 때이다. 그리고 키보드로 kill, break를 치는 경우도 해당한다. 부모가 종료하는 경우에 부모 프로세스가 종료하기 전에 자식들이 먼저 종료된다
...
- 프로세스 간 협력 메커니즘(IPC)
  - 메시지를 전달하는 방법
    - message passing : 커널을 통해 메시지 전달
      - 직접 통신 : 통신하려는 프로세스 이름을 명시적으로 표시
      - 간접 통신 : mailbox 또는 port를 통해 메시지를 간접 전달
  - 주소 공간을 공유하는 방법
    - shared memory : 서로 다른 프로세스 간에도 일부 주소 공간을 공유하게 하는 shared memory 메커니즘이 있다
    - thread : 스레드는 사실상 하나의 프로세스이므로 프로세스 간 협력으로 보기는 어렵지만 동일한 process를 구성하는 thread 간에는 주소 공간을 공유하므로 협력이 가능

![image](https://user-images.githubusercontent.com/67304980/140392480-567c2a04-2fdf-45f9-80a7-69a144531d1f.png)
